					...
					<table class="table">
						<thead>
							<tr>
								...
								<th>Time</th>
								<th>Status</th>
								<th><span>Click to perform action</span></th>
							</tr>
						</thead>
						<tbody>
							<?php
                                    ...
                                    if(isset($_GET['ud']))
                                    {
                                        ...
                                        $update = mysqli_query($server, "UPDATE `lost_ids` SET `status`='Owner Found' WHERE id='$ud'") or die('Error booking tasks');
                                                if ($update) {
                                                  echo "<div class='alert alert-block alert-success'>
                                                   <button type='button' class='close' data-dismiss='alert'>×</button>
									 ...
									 if(isset($_GET['dd']))
                                    {
                                        $dd = $_GET['dd'];
                                                ...
                                                $delete = mysqli_query($server, "DELETE FROM `lost_ids` WHERE id='$dd'") or die('Error');
                                                if ($delete) {
                                                  echo "<div class='alert alert-block alert-success'>
                                                   <button type='button' class='close' data-dismiss='alert'>×</button>
                        ...
                        $select = mysqli_query($server, "SELECT * FROM `lost_ids` ORDER BY `REG_NO`DESC ") or die('Mysql Error');
                            while ($column=mysqli_fetch_array($select)) {
                            	...
                            	<tr>
                            		...
                            		<td>$column[8]</td>
                            		<td>$column[9]</td>
                            		<td><a class='btn btn-mini btn-info' href='mylostfoundids.php?ud=$column[0]'>Owner Found</a>
                            		<br><br><a class='btn btn-mini btn-danger' href='mylostfoundids.php?dd=$column[0]'>Remove</a></td>
                            	</tr>
                            	";
                            }
							?>
