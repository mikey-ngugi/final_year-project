	...
	<div class="tutorial" style="width: 100%; min-height: 300px;padding: 10px; float: left;">
		...
		<h1>My Applied ID</h1>
					<table class="table">
                        ...
                        $select = mysqli_query($server, "SELECT * FROM `apply_id` WHERE `email`='$user_email' ORDER BY `reg_no`ASC ") or die('Mysql Error');
                            while ($column=mysqli_fetch_array($select)) {
                                ...
                                <td>
                                <br><br>
                                <a class='btn btn-mini btn-danger' href='myapplication.php?dd=$column[0]'>Delete</a></td></td>
                            	</tr>
                            	";
                            }
							?>
